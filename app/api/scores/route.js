...
// Polls ESPN's public scoreboard endpoint for live NCAA tournament scores
// Returns player box score data matched to our draft pool

import { PICKS, DRAFTERS, TEAM_IDS } from '../../data';
...
// Build a lookup of all drafted player names -> { drafter, playerIndex }
...
// Try to match an ESPN player name to our draft pool
...
export async function GET(request) {
  try {
    ...
    const group = searchParams.get('group') || '100'; // 100 = NCAA tournament
    ...
    const sbRes = await fetch(url, { next: { revalidate: 30 } });
    const sbData = await sbRes.json();
    const events = sbData?.events || [];

    const lookup = buildPlayerLookup();
    const playerScores = {}; // { "drafter:idx" -> { pts, games: [...] } }
    const liveGames = [];
    ...
    for (const event of events) {
      ...
      // Fetch box score for live/completed games
      try {
        const boxRes = await fetch(ESPN_BOXSCORE(gameId), { next: { revalidate: 30 } });
        ...
        console.error(`Failed to fetch box score for game ${gameId}:`, e.message);
      }
    }

    // Build tournament progress
    const totalGames = 67; // Total tournament games
